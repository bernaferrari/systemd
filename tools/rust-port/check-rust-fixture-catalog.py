...
def rust_linked_fixtures(root: Path) -> list[tuple[str, str]]:
    ...
    for match in re.finditer(r"\bexecutable\s*\(", text):
        ...
        if target_match is None or not re.search(
        ...
        if len(sources) != 1:
            raise ValueError(f"{target}: expected exactly one literal test-*.c fixture, got {sources}")
