...
class UnsafeSafetyGateTests(unittest.TestCase):
    ...
    def test_multiline_unsafe_construct_is_counted(self) -> None:
        ...
        result = self.run_gate("--baseline", "baseline.json")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("fixture.rs,1,0,0,1,1,OK", result.stdout)
    ...
    def test_type_alias_to_unsafe_function_pointer_is_not_a_site(self) -> None:
        ...
        result = self.run_gate("--baseline", "baseline.json")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("unsafe_sites=0 api_sites=0 abi_sites=0 missing_safety=0", result.stdout)
    ...
    def test_unsafe_extern_is_reported_separately_from_execution_surface(self) -> None:
        ...
        result = self.run_gate("--baseline", "baseline.json")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("fixture.rs,0,0,1,0,0,OK", result.stdout)

    def test_unsafe_function_is_reported_as_api_contract(self) -> None:
        self.source.write_text(
            "// SAFETY: fixture caller contract.\n"
            "unsafe fn imported(value: *mut u8) { let _ = value; }\n",
            encoding="utf-8",
        )
        self.write_baseline()
        ...
        result = self.run_gate("--baseline", "baseline.json")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("fixture.rs,0,1,0,0,0,OK", result.stdout)
