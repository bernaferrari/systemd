...
#define rs_macro(x) ((x) + 1)
    ...
    predicate = (
        '"rs_in4_addr_is_null", ffi_in4_addr_is_null, '
        "a: *const InAddr, InAddr, rs_in4_addr_is_null"
    )
    if addr_predicate_signature(predicate) != (
        "rs_in4_addr_is_null",
        (("*constInAddr",), "bool"),
    ):
        raise ValueError("predicate forwarding ABI parser self-check failed")
    try:
        addr_predicate_signature('"rs_predicate", wrapper, a: *const InAddr, InAddr')
    except ValueError:
        pass
    else:
        raise ValueError("predicate forwarding ABI parser accepted a truncated invocation")
...
def addr_predicate_signature(body: str) -> tuple[str, Signature]:
    """Parse the one-pointer, bool ABI generated by the predicate facade.

    Unlike ``ffi_forward!``, the implementation name is normally identical to
    the exported symbol, so counting all ``rs_*`` names would collapse the
    invocation to one name. Keep this as a separate, exact grammar rather
    than relaxing the two-name rule used by the other forwarding macros.
    """

    match = re.fullmatch(
        r'\s*"(?P<name>rs_[A-Za-z0-9_]+)"\s*,\s*'
        r"[A-Za-z_][A-Za-z0-9_]*\s*,\s*"
        r"[A-Za-z_][A-Za-z0-9_]*\s*:\s*(?P<parameter>[^,]+?)\s*,\s*"
        r"[A-Za-z_][A-Za-z0-9_]*\s*,\s*"
    ...
    if not match:
        raise ValueError("cannot parse ffi_forward_addr_predicate! export signature")
    return (
        match.group("name"),
        ((canonical_rust_type(match.group("parameter")),), "bool"),
    )
    ...
    for source_path in source_paths:
        ...
        for macro_match in inventory.MACRO_INVOCATION_RE.finditer(source):
            ...
            if macro == "ffi_forward":
                ...
                continue
            if macro == "ffi_forward_addr_predicate":
                name, signature = addr_predicate_signature(body)
                if names != [name]:
                    raise ValueError(
                        "ffi_forward_addr_predicate! must declare exactly one "
                        f"Rust ABI name, found {names}"
                    )
