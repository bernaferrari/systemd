...
pub fn build_net_driver_properties(driver: &str, ifname: &str) -> Result<BTreeMap<String, String>> {
    if driver.trim().is_empty() {
        return Err(NetDriverError::EmptyDriver);
    }
    Ok(BTreeMap::from([
        ("ID_NET_DRIVER".into(), driver.trim().into()),
        ("INTERFACE".into(), ifname.trim().into()),
    ]))
}
...
mod tests {
    use super::*;
    #[test]
    fn exports_driver_property() {
        let props = build_net_driver_properties("e1000e", "eth0").unwrap();
        assert_eq!(props["ID_NET_DRIVER"], "e1000e");
    }
    #[test]
    fn rejects_empty_driver() {
        assert_eq!(
            build_net_driver_properties(" ", "eth0"),
            Err(NetDriverError::EmptyDriver)
        );
    }
}
