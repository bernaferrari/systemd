...
pub fn plan_kmod_load(module: &str, builtin_modules: &[&str]) -> Result<KmodRequest> {
    ...
    if module.is_empty() {
        return Err(KmodError::EmptyModule);
    }
    Ok(KmodRequest {
        module: module.into(),
...
mod tests {
    use super::*;
    #[test]
    fn marks_builtin_module() {
        let request = plan_kmod_load("loop", &["loop", "vfat"]).unwrap();
        assert!(request.builtin);
    }
    #[test]
    fn rejects_empty_module() {
        assert_eq!(plan_kmod_load("", &[]), Err(KmodError::EmptyModule));
    }
}
