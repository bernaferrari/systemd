...
/// Safe internal implementation of format_timespan.
fn format_timespan_parts(t: u64, accuracy: u64) -> Vec<String> {
    if t == USEC_INFINITY {
        return Vec::new();
    }
    ...
    for entry in TIMESPAN_TABLE.iter() {
        if t == 0 {
            break;
        }

        if !parts.is_empty() && t < accuracy {
            break;
        }
        ...
        let a = t / entry.usec;
        let mut b = t % entry.usec;

        let suffix = std::str::from_utf8(entry.suffix).unwrap_or("");

        if t < USEC_PER_MINUTE && b > 0 {
            let mut j: i32 = 0;
            let mut cc: u64 = entry.usec;
            while cc > 1 {
                cc /= 10;
                j += 1;
            }
