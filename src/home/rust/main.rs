...
//
// Fail-closed entry point for systemd-homed.
//
...
const EXECUTION_UNAVAILABLE: &str = concat!(
    "systemd-homed: daemon execution is unavailable in the Rust port; ",
    "refusing to claim readiness without the canonical D-Bus, Varlink, home, and worker lifecycle"
);
...
fn print_help() {
    println!("systemd-homed [--help|--version]");
    println!();
    println!("The Rust systemd-homed executable is unavailable until it implements");
    println!("the complete canonical D-Bus, Varlink, home, and worker lifecycle.");
}

fn print_version() {
    println!("systemd-homed {}", VERSION);
}
