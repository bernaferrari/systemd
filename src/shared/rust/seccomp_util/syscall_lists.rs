...
const SYSCALL_LIST: &str = include_str!("../../../include/override/sys/syscall-list.txt");
static KNOWN_SYSCALLS: OnceLock<Vec<&'static str>> = OnceLock::new();

fn known_syscalls() -> &'static [&'static str] {
    KNOWN_SYSCALLS
        .get_or_init(|| {
            std::iter::once("@obsolete")
                .chain(SYSCALL_LIST.lines())
                .collect()
        })
        .as_slice()
}
...
pub(super) fn syscalls(set: SyscallFilterSet) -> &'static [&'static str] {
    match set {
        ...
        SyscallFilterSet::Known => known_syscalls(),
    }
}
